...
module libvpx.vpx.vp8cx;

import libvpx.vpx.vp8;
...
// TODO: I commented these iface here since I don't need them (yet).
 ...
 */
enum VP8_EFLAG_NO_REF_LAST     = (1<<16);
 ...
 */
enum VP8_EFLAG_NO_REF_ARF      = (1<<21);
 ...
 */
enum VP8_EFLAG_NO_UPD_LAST     = (1<<18);
 ...
 * the contents of the current frame.
 */
enum VP8_EFLAG_NO_UPD_ARF      = (1<<23);
 ...
 */
enum VP8_EFLAG_NO_UPD_ENTROPY  = (1<<20);
...
alias VPX_SCALING_MODE = vpx_scaling_mode_1d;
...
struct vpx_roi_map {
  ...
  uint static_threshold[4];
} 
alias vpx_roi_map_t = vpx_roi_map;
...
struct vpx_active_map {
  ubyte  *active_map; /**< specify an on (1) or off (0) each 16x16 region within a frame */
  uint    rows;       /**< number of rows */
  uint    cols;       /**< number of cols */
}
alias vpx_active_map_t = vpx_active_map;
...
alias vpx_scaling_mode_t = vpx_scaling_mode;
...
struct vpx_svc_parameters {
  uint width;         /**< width of current spatial layer */
  uint height;        /**< height of current spatial layer */
...
alias vpx_svc_parameters_t = vpx_svc_parameters;

struct vpx_svc_layer_id {
  int spatial_layer_id;
  int temporal_layer_id;
}
alias vpx_svc_layer_id_t = vpx_svc_layer_id;
...
VPX_CTRL_USE_TYPE(VP8E_SET_ENABLEAUTOALTREF,   uint)
VPX_CTRL_USE_TYPE(VP8E_SET_NOISE_SENSITIVITY,  uint)
VPX_CTRL_USE_TYPE(VP8E_SET_SHARPNESS,          uint)
VPX_CTRL_USE_TYPE(VP8E_SET_STATIC_THRESHOLD,   uint)
VPX_CTRL_USE_TYPE(VP8E_SET_TOKEN_PARTITIONS,   int) // vp8e_token_partitions

VPX_CTRL_USE_TYPE(VP8E_SET_ARNR_MAXFRAMES,     uint)
VPX_CTRL_USE_TYPE(VP8E_SET_ARNR_STRENGTH,     uint)
VPX_CTRL_USE_TYPE(VP8E_SET_ARNR_TYPE,     uint)
VPX_CTRL_USE_TYPE(VP8E_SET_TUNING,             int) // vp8e_tuning
VPX_CTRL_USE_TYPE(VP8E_SET_CQ_LEVEL,      uint)
...
VPX_CTRL_USE_TYPE(VP8E_SET_MAX_INTRA_BITRATE_PCT, uint)

VPX_CTRL_USE_TYPE(VP9E_SET_LOSSLESS, uint)

VPX_CTRL_USE_TYPE(VP9E_SET_FRAME_PARALLEL_DECODING, uint)

VPX_CTRL_USE_TYPE(VP9E_SET_AQ_MODE, uint)
*/
